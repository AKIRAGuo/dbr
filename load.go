...
func Load(rows *sql.Rows, value interface{}, d Dialect) (int, error) {
	defer rows.Close()
	...
	for rows.Next() {
		...
		if isMap {
			if (d == dialect.ACI) {
				err := s.findACIPtr(elem, column[1:], ptr[1:])
				if err != nil {
					return 0, err
				}
				keyElem = reflectAlloc(v.Type().Key())
				err = s.findACIPtr(keyElem, column[:1], ptr[:1])
				if err != nil {
					return 0, err
				}
			}else {
			...
			if (d == dialect.ACI) {
				err := s.findACIPtr(elem, column, ptr)
				if err != nil {
					return 0, err
				}
			}else {
