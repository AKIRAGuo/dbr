...
var ACINameMapping = camelCaseToUpperSnakeCase
...
func camelCaseToUpperSnakeCase(name string) string {
	snakeCase := camelCaseToSnakeCase(name)
	var buf strings.Builder
	buf.Grow(len(snakeCase) * 2)
	for i := 0; i < len(snakeCase); i++ {
		buf.WriteByte(toUpper(snakeCase[i]))
	}
	return buf.String()
}
...
func (s *tagStore) findACIPtr(value reflect.Value, name []string, ptr []interface{}) error {
	if value.CanAddr() && value.Addr().Type().Implements(typeScanner) {
		ptr[0] = value.Addr().Interface()
		return nil
	}
	switch value.Kind() {
	case reflect.Struct:
		s.findACIValueByName(value, name, ptr, true)
		return nil
	case reflect.Ptr:
		if value.IsNil() {
			value.Set(reflect.New(value.Type().Elem()))
		}
		return s.findACIPtr(value.Elem(), name, ptr)
	default:
		ptr[0] = value.Addr().Interface()
		return nil
	}
	...
	case reflect.Struct:
		...
		for i := 0; i < value.NumField(); i++ {
			...
			for i, want := range name {
				if strings.ToLower(want) != strings.ToLower(tag) {
					continue
				}
				if ret[i] == nil {
					if retPtr {
...
func (s *tagStore) findACIValueByName(value reflect.Value, name []string, ret []interface{}, retPtr bool) {
	if value.Type().Implements(typeValuer) {
		return
	}
	switch value.Kind() {
...
func (s *tagStore) getACI(t reflect.Type) []string {
	if t.Kind() != reflect.Struct {
		return nil
	}
	if _, ok := s.m[t]; !ok {
		...
		for i := 0; i < t.NumField(); i++ {
			field := t.Field(i)
			if false && field.PkgPath != "" && !field.Anonymous {
				// unexported
				continue
			}
			tag := field.Tag.Get("db")
